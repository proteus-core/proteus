...
class Pipeline(val config: Config) extends Component {
  private val plugins = mutable.ArrayBuffer[Plugin]()
  ...
  def addPlugins(plugins: Seq[Plugin]): Unit = {
